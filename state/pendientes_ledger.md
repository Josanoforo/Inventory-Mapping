...
| P-134 | `evidence_role` sí cruza a Phase 3 — ER→SC lo conserva sin renombrar (`Signal Card.evidence_role`); el renombrado a `evidence_base` ocurre en el paso de renderizado a markdown (`signal_to_markdown.py:274`), no en un schema. `card_record.schema.json` declara `evidence_base` como string libre sin enum: el valor cruza, el enum no | decisión | Extender el puente para que el enum sobreviva al renderizado (precedente D-126), o aceptar que `evidence_base` quede como texto libre | DSC | verificado — `state/output/field_lifecycle_card_record_bridge.md:18` (rama `claude/field-lifecycle-population-audit-a060e7`, Run 3) |
